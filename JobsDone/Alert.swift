...
extension Alertift {
    ...
    final public class Alert: AlertType, _AlertType {
        ...
        /// - Returns: Myself
        final public func action(_ action: Alertift.Action, isPreferred: Bool = false, textFieldsHandler handler: @escaping () -> Void) -> Self {
            addActionToAlertController(
                buildAlertAction(action, handler: { //_ in
                    handler()
                }),
