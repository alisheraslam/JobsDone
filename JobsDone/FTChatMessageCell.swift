...
class FTChatMessageCell: UITableViewCell {
    ...
    convenience init(style: UITableViewCell.CellStyle, reuseIdentifier: String?, theMessage : FTChatMessageModel, for indexPath: IndexPath, lastIndex: Int) {
        ...
        if indexPath.row == 0 {
            self.addTimeLabel(isUserSelf: message.isUserSelf)
        }
...
//        }
    ...
    func setupCellBubbleItem(_ bubbleFrame: CGRect, for indexPath: IndexPath) {
        ...
        messageBubbleItem.addTarget(self, action: #selector(self.itemTapped), for: UIControl.Event.touchUpInside)
    ...
    //MARK: - addTimeLabel
    func addTimeLabel(isUserSelf: Bool) {
        if isUserSelf {
            messageTimeLabel.textAlignment = .right
            let timeLabelRect = CGRect(x: -40, y: -FTDefaultMessageCellTimeLabelHeight ,width: FTScreenWidth, height: FTDefaultMessageCellTimeLabelHeight + 20)
